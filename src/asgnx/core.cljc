...
;; complete specification. {:keys [args cmd]}
;;
(defn office-hours [days]
  (if (contains? instructor-hours (first (get days :args)))
      (formatted-hours (get instructor-hours (first(get days :args))))
...
;;
(defn experts-register [experts topic expert-id expert-info]
  (action-insert [:expert topic expert-id] expert-info))

;;(action-insert expert [topic expert-id]))
;;(action-inserts experts [:expert topic expert-id] expert-info))
...
(def routes {"default"  (stateless (fn [& args] "Unknown test."))
...
(defn create-router [routes]
  (fn [msg]
    (if (contains? routes (msg :cmd))
      (get routes (get msg :cmd) (get msg :args))
      (get routes "default"))))
